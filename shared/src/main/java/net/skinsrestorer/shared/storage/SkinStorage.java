...
public class SkinStorage implements ISkinStorage {
    ...
    public boolean purgeOldSkins(int Days) {
        ...
        if (Config.MYSQL_ENABLED) {
            ...
            mysql.execute("DELETE FROM " + Config.MYSQL_PLAYER_TABLE + " WHERE Nick NOT LIKE ' %' AND " + Config.MYSQL_PLAYER_TABLE + ".timestamp<>0 AND " + Config.MYSQL_PLAYER_TABLE + ".timestamp<=?", targetPurgeTimestamp);
            return true;
        } else {
            List<Path> files = new ArrayList<>();
            try (DirectoryStream<Path> stream = Files.newDirectoryStream(skinsFolder, "*.skin")) {
